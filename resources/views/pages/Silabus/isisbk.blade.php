...
>>>>>>> origin/master
                                                <div class="btn-group">
                                                    <a href="{{  route('silabussbk') }}" id="addRow" class="btn btn-primary">
                                        ...
                                        @foreach($sbksilabus as $rowsbk)
										<table>
												<tbody>
													<tr>
														<td>
															<h4>Download Silabus: <a href="{{ asset('filesilabus/'.$rowsbk->file) }}"> {{ $rowsbk->nama_materi }} </a></h4>
															<h5>Mengenai Silabus: {{ $rowsbk->silabus }}</h5>
														</td>
														@auth
														@if(Auth::user()->role == 'admin')
														<td>															
															<a href="{{ route('editsilabussbk', $rowsbk->id) }}" class="tblEditBtn">
																<i class="fa fa-pencil"></i>
															</a>
															<a href="#" class="tblDelBtn delete" data-id="{{ $rowsbk->id }}">
...
<script>
	$('.delete').click(function() {
		var silabussbkid = $(this).attr('data-id');
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					window.location = "/deletesilabussbk/" + silabussbkid;
